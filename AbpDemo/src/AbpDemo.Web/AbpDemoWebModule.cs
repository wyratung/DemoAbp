...
using AbpDemo.Web.HealthChecks;
...
public class AbpDemoWebModule : AbpModule
{
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        Configure<AbpVirtualFileSystemOptions>(options =>
        {
            ...
            if (hostingEnvironment.IsDevelopment())
            {
                options.FileSets.ReplaceEmbeddedByPhysical<AbpDemoDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AbpDemo.Domain.Shared", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpDemoDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AbpDemo.Domain", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpDemoApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AbpDemo.Application.Contracts", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpDemoApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}AbpDemo.Application", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpDemoHttpApiModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}AbpDemo.HttpApi", Path.DirectorySeparatorChar)));
    ...
    private void ConfigureNavigationServices()
    {
        ...
        {
            options.Contributors.Add(new AbpDemoToolbarContributor());
        });
    }
